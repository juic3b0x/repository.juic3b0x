...
class Navigator:
	...
	def root(self):
		...
		if getMenuEnabled('navi.changelog'): self.addDirectoryItem(32014, 'tools_ShowChangelog&name=wutu', 'icon.png', 'DefaultAddonHelper.png', isFolder=False)
		self.endDirectory()
	...
	def traktSearchLists(self, media_type):
		...
		control.closeAll()
		if media_type == 'movies': control.execute('ActivateWindow(Videos,plugin://plugin.video.wutu/?action=movies_PublicLists&url=%s,return)' % (quote_plus(url)))
		else: control.execute('ActivateWindow(Videos,plugin://plugin.video.wutu/?action=tv_PublicLists&url=%s,return)' % (quote_plus(url)))
	...
	def tools(self):
		...
		self.addDirectoryItem(32506, 'tools_contextwutuSettings', 'icon.png', 'DefaultAddonProgram.png', isFolder=False)
		#-- Providers - 4
		self.addDirectoryItem(32651, 'tools_ezscrapersSettings', 'ezscrapers.png', 'DefaultAddonService.png', isFolder=False)
	...
	def loggingNavigator(self):
		self.addDirectoryItem(32524, 'tools_viewLogFile&name=wutu', 'tools.png', 'DefaultAddonProgram.png', isFolder=False)
		self.addDirectoryItem(32525, 'tools_clearLogFile', 'tools.png', 'DefaultAddonProgram.png', isFolder=False)
		self.addDirectoryItem(32526, 'tools_ShowChangelog&name=wutu', 'tools.png', 'DefaultAddonProgram.png', isFolder=False)
		self.addDirectoryItem(32527, 'tools_uploadLogFile&name=wutu', 'tools.png', 'DefaultAddonProgram.png', isFolder=False)
		...
		self.addDirectoryItem(32530, 'tools_viewLogFile&name=ezscrapers', 'tools.png', 'DefaultAddonProgram.png', isFolder=False)
		self.addDirectoryItem(32531, 'tools_ShowChangelog&name=ezscrapers', 'tools.png', 'DefaultAddonProgram.png', isFolder=False)
	...
	def views(self):
		try:
			...
			title = getLS(32059)
			url = 'plugin://plugin.video.wutu/?action=tools_addView&content=%s' % content
	...
	def addDirectoryItem(self, name, query, poster, icon, context=None, queue=False, isAction=True, isFolder=True, isPlayable=False, isSearch=False, table=''):
		try:
			...
			url = 'plugin://plugin.video.wutu/?action=%s' % query if isAction else query
			...
			if queue: cm.append((queueMenu, 'RunPlugin(plugin://plugin.video.wutu/?action=playlist_QueueItem)'))
			if context: cm.append((getLS(context[0]), 'RunPlugin(plugin://plugin.video.wutu/?action=%s)' % context[1]))
			if isSearch: cm.append(('Clear Search Phrase', 'RunPlugin(plugin://plugin.video.wutu/?action=cache_clearSearchPhrase&source=%s&name=%s)' % (table, quote_plus(name))))
			cm.append(('[COLOR lime]wutu Settings[/COLOR]', 'RunPlugin(plugin://plugin.video.wutu/?action=tools_openSettings)'))
