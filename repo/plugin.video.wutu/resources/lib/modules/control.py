...
def setting(id, fallback=None):
	try: settings_dict = jsloads(homeWindow.getProperty('wutu_settings'))
...
def make_settings_dict(): # service runs upon a setting change
	try:
		...
		homeWindow.setProperty('wutu_settings', jsdumps(settings_dict))
		refresh_playAction()
		refresh_libPath()
...
def getwutuVersion():
	return xbmcaddon.Addon('plugin.video.wutu').getAddonInfo('version')
...
def artPath():
	theme = appearance()
	return joinPath(xbmcaddon.Addon('plugin.video.wutu').getAddonInfo('path'), 'resources', 'artwork', theme)

def genreIconPath():
	return joinPath(xbmcaddon.Addon('plugin.video.wutu').getAddonInfo('path'), 'resources', 'artwork', 'genre_media', 'icons')

def genrePosterPath():
	return joinPath(xbmcaddon.Addon('plugin.video.wutu').getAddonInfo('path'), 'resources', 'artwork', 'genre_media', 'posters')
...
def getColor(n):
	colorChart = ('blue', 'red', 'yellow', 'deeppink', 'cyan', 'lawngreen', 'gold', 'lime', 'yellowgreen',
...
def refresh_playAction(): # for wutu global CM play actions
	autoPlay = 'true' if setting('play.mode') == '1' else ''
	homeWindow.setProperty('wutu.autoPlay.enabled', autoPlay)

def refresh_libPath(): # for wutu global CM library actions
	homeWindow.setProperty('wutu.movieLib.path', transPath(setting('library.movie')))
	homeWindow.setProperty('wutu.tvLib.path', transPath(setting('library.tv')))

def refresh_debugReversed(): # called from service "onSettingsChanged" to clear wutu.log if setting to reverse has been changed
	if homeWindow.getProperty('wutu.debug.reversed') != setting('debug.reversed'):
		homeWindow.setProperty('wutu.debug.reversed', setting('debug.reversed'))
		execute('RunPlugin(plugin://plugin.video.wutu/?action=tools_clearLogFile)')
