...
def router(params):
	...
	if action is None:
		...
		isUpdate = control.homeWindow.getProperty('wutu.updated')
		if isUpdate == 'true':
			control.execute('RunPlugin(plugin://plugin.video.wutu/?action=tools_cleanSettings)')
			control.homeWindow.clearProperty('wutu.updated')
	...
	####################################################
		...
		if action == 'tools_ShowNews':
			...
			if params.get('opensettings') == 'true':
				control.openSettings(params.get('query2'), 'plugin.video.wutu')
		elif action == 'tools_syncMyAccount':
			from resources.lib.modules import my_accounts
			my_accounts.syncMyAccounts()
			if params.get('opensettings') == 'true':
				control.openSettings(query, 'plugin.video.wutu')
		...
		elif action == 'tools_contextwutuSettings':
			control.openSettings('0.0', 'context.wutu')
			control.trigger_widget_refresh()
		elif action == 'tools_ezscrapersSettings':
			...
			elif cleared: control.notification(message='wutu Log File Successfully Cleared')
			else: control.notification(message='Error clearing wutu Log File, see kodi.log for more info')
	...
	####################################################
		...
		if action == 'play_Item':
			...
			for i in items:
				...
				url = 'plugin://plugin.video.wutu/?action=play_Item&title=%s&year=%s&imdb=%s&tmdb=%s&tvdb=%s&season=%s&episode=%s&tvshowtitle=%s&premiered=%s&meta=%s&select=1' % (
			...
			if rtype == 'movie':
				...
				r = 'plugin://plugin.video.wutu/?action=play_Item'
			elif rtype == 'episode':
				from resources.lib.menus import episodes
				...
				r = 'plugin://plugin.video.wutu/?action=play_Random&rtype=episode'
			elif rtype == 'show':
				from resources.lib.menus import tvshows
				rlist = tvshows.TVshows().get(url, create_directory=False)
				r = 'plugin://plugin.video.wutu/?action=play_Random&rtype=season'
	...
	####################################################
		...
		elif action == 'cache_clearKodiBookmark': # context.wutu action call only
