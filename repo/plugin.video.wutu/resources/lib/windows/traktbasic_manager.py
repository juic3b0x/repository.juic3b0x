		...
		try:
			if action in self.selection_actions:
				...
				if focus_id == 2050: # listItems
					...
					trakt = chosen_listitem.getProperty('wutu.trakt')
					...
					for item in self.item_list:
						...
						self.selected_items.append(item.getProperty('wutu.trakt'))
				...
				media_type = chosen_listitem.getProperty('wutu.media_type')
				source_trailer = chosen_listitem.getProperty('wutu.trailer')
				if not source_trailer:
					from resources.lib.modules import trailer
					if media_type == 'show':
						source_trailer = trailer.Trailer().worker('show', chosen_listitem.getProperty('wutu.tvshowtitle'), chosen_listitem.getProperty('wutu.year'), None, chosen_listitem.getProperty('wutu.imdb'))
					else:
						source_trailer = trailer.Trailer().worker('movie', chosen_listitem.getProperty('wutu.title'), chosen_listitem.getProperty('wutu.year'), None, chosen_listitem.getProperty('wutu.imdb'))
				...
				if cm_action == 'browseSeries':
					systvshowtitle = quote_plus(chosen_listitem.getProperty('wutu.tvshowtitle'))
					year = chosen_listitem.getProperty('wutu.year')
					imdb = chosen_listitem.getProperty('wutu.imdb')
					tmdb = chosen_listitem.getProperty('wutu.tmdb')
					tvdb = chosen_listitem.getProperty('wutu.tvdb')
					...
					self.close()
					sysart = quote_plus(chosen_listitem.getProperty('wutu.art'))
					self.execute_code('ActivateWindow(Videos,plugin://plugin.video.wutu/?action=seasons&tvshowtitle=%s&year=%s&imdb=%s&tmdb=%s&tvdb=%s&art=%s,return)' % (
		...
		def builder():
			for count, item in enumerate(self.results, 1):
				try:
					...
					listitem.setProperty('wutu.title', item.get('title'))
					if item.get('tvshowtitle'): listitem.setProperty('wutu.media_type', 'show')
					else: listitem.setProperty('wutu.media_type', 'movie')
					listitem.setProperty('wutu.year', str(item.get('year')))
					listitem.setProperty('wutu.isSelected', '')
					listitem.setProperty('wutu.imdb', item.get('imdb'))
					listitem.setProperty('wutu.tmdb', item.get('tmdb'))
					listitem.setProperty('wutu.tvdb', item.get('tvdb'))
					listitem.setProperty('wutu.trakt', item.get('trakt'))
					listitem.setProperty('wutu.status', item.get('status'))
					listitem.setProperty('wutu.rating', str(round(float(item.get('rating', '0')), 1)))
					listitem.setProperty('wutu.trailer', item.get('trailer'))
					listitem.setProperty('wutu.studio', item.get('studio'))
					listitem.setProperty('wutu.genre', item.get('genre', ''))
					listitem.setProperty('wutu.duration', str(item.get('duration')))
					listitem.setProperty('wutu.mpaa', item.get('mpaa') or 'NA')
					listitem.setProperty('wutu.plot', item.get('plot'))
					...
					listitem.setProperty('wutu.poster', poster)
					listitem.setProperty('wutu.clearlogo', clearlogo)
					listitem.setProperty('wutu.art', jsdumps(art))
