			...
			if action in self.selection_actions:
				...
				if focus_id == 2060: # listItems
					...
					if chosen_listitem.getProperty('wutu.isSelected') == 'true':
						...
						season = chosen_listitem.getProperty('wutu.season')
						episode = chosen_listitem.getProperty('wutu.episode')
					...
					for item in self.item_list:
						...
						self.selected_items.append({'imdb': item.getProperty('wutu.imdb'), 'tvdb': item.getProperty('wutu.tvdb'),
							'season': item.getProperty('wutu.season'), 'episode': item.getProperty('wutu.episode')})
		...
		def builder():
			for count, item in enumerate(self.results, 1):
				try:
					...
					listitem.setProperty('wutu.tvshowtitle', tvshowtitle)
					...
					listitem.setProperty('wutu.premiered', new_date)
					season = str(item.get('season'))
					listitem.setProperty('wutu.season', season)
					episode = str(item.get('episode'))
					listitem.setProperty('wutu.episode', episode)
					label = '%s  -  %sx%s' % (tvshowtitle, season, episode.zfill(2))
					listitem.setProperty('wutu.label', label)
					labelProgress = str(round(float(item['progress']), 1)) + '%'
					listitem.setProperty('wutu.progress', '[' + labelProgress + ']')
					...
					listitem.setProperty('wutu.genre', str(item.get('genre', '')))
