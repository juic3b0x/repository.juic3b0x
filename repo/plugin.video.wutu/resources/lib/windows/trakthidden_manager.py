		...
		try:
			if action in self.selection_actions:
				...
				if focus_id == 2040: # listItems
					...
					if chosen_listitem.getProperty('wutu.isHidden') == 'true':
						...
						if chosen_listitem.getProperty('wutu.isSelected') == '':
							chosen_listitem.setProperty('wutu.isSelected', 'true')
		...
		def builder():
			for count, item in enumerate(self.results, 1):
				try:
					...
					listitem.setProperty('wutu.tvshowtitle', item.get('tvshowtitle'))
					listitem.setProperty('wutu.year', str(item.get('year')))
					listitem.setProperty('wutu.isHidden', str(item.get('isHidden')))
					listitem.setProperty('wutu.isSelected', str(item.get('isHidden')))
					listitem.setProperty('wutu.imdb', item.get('imdb'))
					listitem.setProperty('wutu.tmdb', str(item.get('tmdb')))
					listitem.setProperty('wutu.tvdb', str(item.get('tvdb')))
					listitem.setProperty('wutu.status', item.get('status'))
					try: listitem.setProperty('wutu.watched_count', '(watched ' + str(item.get('watched_count').get('watched')) + ' of ' + str(item.get('watched_count').get('total')) + ')')
					except: pass
					...
					# listitem.setProperty('wutu.duration', str(item.get('duration'))) # not used
